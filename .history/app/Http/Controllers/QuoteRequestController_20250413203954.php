...
class QuoteRequestController extends Controller
{
    ...
    /**
     * Show the form for creating a quote request for a specific product.
     */
    public function productQuote($id)
    {
        $product = Product::findOrFail($id);
        return view('quote-request.product-quote', compact('product'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Redirect dengan pesan sukses dan informasi tambahan
        return redirect()->route('home')->with([
            'success' => 'Permintaan penawaran Anda telah berhasil dikirim!',
            'quote_info' => [
                'name' => $validated['name'],
                'email' => $validated['email'],
                'created_at' => now()->format('d M Y, H:i'),
                'products_count' => count($productData),
                'reference_number' => 'QR-' . $quoteRequest->id . '-' . now()->format('Ymd'),
            ]
        ]);
    }
}
