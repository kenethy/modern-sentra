...
{{-- Define JS variables based on Blade condition BEFORE the main script --}}
<script>
    const initialSelectedProductId = @json(isset($selectedProduct) ? $selectedProduct->id : null);
    const initialSelectedProductName = @json(isset($selectedProduct) ? $selectedProduct->name : null);
</script>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Pre-select product if exists - Moved inside DOMContentLoaded
        @if (isset($selectedProduct) && $selectedProduct)
            // Ensure the element exists before trying to select it
            const initialProductElement = document.querySelector(`.product-item[data-id="{{ $selectedProduct->id }}"]`);
            if (initialProductElement) {
                 toggleProductSelection('{{ $selectedProduct->id }}', '{{ $selectedProduct->name }}', initialProductElement);
            } else {
                // If element not found immediately (e.g., in collapsed category), add to selectedProducts directly
                // and the UI will update correctly when the category is expanded or filters change.
                 selectedProducts['{{ $selectedProduct->id }}'] = { id: '{{ $selectedProduct->id }}', name: '{{ $selectedProduct->name }}', quantity: 1 };
                 updateSelectedProductsUI(); // Update UI to show selection even if element isn't visible yet
                 console.log('Pre-selected product added directly to state:', '{{ $selectedProduct->id }}');
            }
        @endif
    });
</script>
