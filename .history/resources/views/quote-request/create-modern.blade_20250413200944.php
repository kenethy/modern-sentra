...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Pre-select product using the JS variables defined above
        if (initialSelectedProductId && initialSelectedProductName) {
            const initialProductElement = document.querySelector(`.product-item[data-id="${initialSelectedProductId}"]`);
            if (initialProductElement) {
                 toggleProductSelection(initialSelectedProductId, initialSelectedProductName, initialProductElement);
            } else {
                // If element not found immediately, add to state directly
                 selectedProducts[initialSelectedProductId] = { id: initialSelectedProductId, name: initialSelectedProductName, quantity: 1 };
                 updateSelectedProductsUI(); // Update UI
                 console.log('Pre-selected product added directly to state:', initialSelectedProductId);
            }
        }
    });
</script>
