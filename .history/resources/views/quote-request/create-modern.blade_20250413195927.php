...
<style>
    ...
    .category-products.expanded {
        max-height: 1000px; /* Adjust if categories have many products */
        opacity: 1;
    }
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12 md:mb-16">
            ...
            <p class="text-base sm:text-lg text-slate-600 max-w-3xl mx-auto">
                Pilih produk konstruksi yang Anda butuhkan, tentukan jumlahnya, lalu isi detail kontak Anda untuk mendapatkan penawaran terbaik dari kami.
            </p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <form id="quoteForm" action="{{ route('quote-request.store') }}" method="POST" class="p-6 sm:p-8 lg:p-10">
                ...
                <div class="mb-10">
                    <div class="flex items-center">
                        ...
                        <div class="flex items-center" id="stepIndicator1">
                            <div class="w-6 h-6 rounded-full bg-[#99765c] text-white flex items-center justify-center text-xs font-bold ring-4 ring-[#99765c]/30 transition-all duration-300">1</div>
                            <span class="ml-3 text-sm font-medium text-[#99765c] transition-colors duration-300">Pilih Produk</span>
                        </div>
                        {{-- Connector --}}
                        <div class="flex-1 h-0.5 mx-4 bg-gradient-to-r from-[#99765c] via-slate-200 to-slate-200 transition-all duration-500" id="progressConnector">
                            <div class="h-full bg-[#99765c] w-0 transition-all duration-500" id="progressBar" style="width: 0%;"></div>
                        </div>
                        {{-- Step 2 --}}
                        <div class="flex items-center" id="stepIndicator2">
                            <div class="w-6 h-6 rounded-full bg-slate-200 text-slate-500 flex items-center justify-center text-xs font-bold ring-4 ring-transparent transition-all duration-300">2</div>
                            <span class="ml-3 text-sm font-medium text-slate-500 transition-colors duration-300">Informasi Kontak</span>
                        </div>
                    </div>
                </div>
                ...
                <div id="step1" class="space-y-8">
                    <h2 class="text-2xl font-semibold text-slate-800 border-b border-slate-200 pb-4">1. Pilih Produk Anda</h2>

                    {{-- Search and Filter --}}
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        <div class="md:col-span-2">
                            <label for="productSearch" class="block text-sm font-medium text-slate-700 mb-1">Cari Produk</label>
                            <div class="relative">
                        ...
                        <div>
                            <label for="categoryFilter" class="block text-sm font-medium text-slate-700 mb-1">Filter Kategori</label>
                    ...
                    <div id="selectedProductsContainer" class="space-y-4 hidden">
                        <h3 class="text-lg font-medium text-slate-700">Produk Dipilih (<span id="selectedCount">0</span>)</h3>
                        <div id="selectedProductsList" class="space-y-3 max-h-60 overflow-y-auto border border-slate-200 rounded-lg p-4 bg-slate-50">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div id="allProductsContainer" class="space-y-1 border border-slate-200 rounded-lg overflow-hidden">
                                @if($categories->isEmpty())
                                    <p class="p-4 text-sm text-slate-500">Tidak ada produk tersedia.</p>
                                @else
                                    @foreach($categories as $category)
                                    <div class="category-section border-b border-slate-200 last:border-b-0" data-category-id="{{ $category->id }}">
                                        <div class="category-header flex items-center justify-between p-4 cursor-pointer hover:bg-slate-50 transition-colors duration-200">
                                            <div class="flex items-center">
                                                <h4 class="text-base font-medium text-slate-800">{{ $category->name }}</h4>
                                                <span class="text-xs text-slate-500 ml-2 bg-slate-200 px-1.5 py-0.5 rounded">({{ $category->products->count() }})</span>
                                            </div>
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-500 transform transition-transform duration-300 arrow-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        ...
                                        <div class="category-products bg-slate-50/50">
                                            <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                @if($category->products->isEmpty())
                                                    <p class="text-sm text-slate-500 italic col-span-full">Tidak ada produk dalam kategori ini.</p>
                                                @else
                                                    @foreach($category->products as $product)
                                                        @include('partials.quote_product_item', ['product' => $product, 'category_id' => $category->id])
                                                    @endforeach
                                                @endif
                                            </div>
                                        </div>
                ...
                <div id="step2" class="space-y-8 hidden">
                     <h2 class="text-2xl font-semibold text-slate-800 border-b border-slate-200 pb-4">2. Informasi Kontak Anda</h2>
                    ...
                    <div class="mb-6 bg-slate-50 rounded-lg p-4 sm:p-6 border border-slate-200">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="button" id="editProductsBtn" class="inline-flex items-center text-sm font-medium text-[#99765c] hover:text-[#876754] hover:underline">
                        ...
                        <div id="productsReview" class="text-sm text-slate-600 divide-y divide-slate-200 max-h-60 overflow-y-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                        ...
                        <div>
                            ...
                            <input type="email" id="email" name="email" required placeholder="contoh: budi.s@example.com"
                                   class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#99765c] focus:border-[#99765c] transition duration-200 text-sm">
                        </div>
                        <div>
                            <label for="phone" class="block text-sm font-medium text-slate-700 mb-1">Nomor Telepon <span class="text-red-500">*</span></label>
                            <input type="tel" id="phone" name="phone" required placeholder="contoh: 081234567890"
                                   class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#99765c] focus:border-[#99765c] transition duration-200 text-sm">
                        </div>
                        <div>
                            <label for="company_name" class="block text-sm font-medium text-slate-700 mb-1">Nama Perusahaan (Opsional)</label>
                            <input type="text" id="company_name" name="company_name" placeholder="contoh: PT Maju Jaya Konstruksi"
                                   class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#99765c] focus:border-[#99765c] transition duration-200 text-sm">
                        </div>
                        <div class="md:col-span-2">
                            <label for="message" class="block text-sm font-medium text-slate-700 mb-1">Pesan / Kebutuhan Detail <span class="text-red-500">*</span></label>
                            ...
                            <p class="mt-1 text-xs text-slate-500">Semakin detail informasi Anda, semakin akurat penawaran yang dapat kami berikan.</p>
                        </div>
                    </div>

                    {{-- Navigation Buttons --}}
                    <div class="flex flex-col sm:flex-row justify-between items-center mt-10 pt-6 border-t border-slate-200 gap-4">
...
<div class="product-item group rounded-lg border border-slate-200 hover:border-[#99765c] hover:bg-amber-50/30 transition-all duration-200 cursor-pointer {{ isset($selectedProducts[$product->id]) ? 'selected border-[#99765c] bg-amber-50/50' : '' }}"
    ...
    <div class="flex items-center p-3 space-x-3">
        ...
        <div class="check-indicator flex items-center justify-center w-6 h-6 rounded-full border-2 border-slate-300 group-hover:border-[#99765c] transition-colors duration-200 {{ isset($selectedProducts[$product->id]) ? 'bg-[#99765c] border-[#99765c]' : 'bg-white' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white {{ isset($selectedProducts[$product->id]) ? '' : 'hidden' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Pre-select product if exists
        @if (isset($selectedProduct) && $selectedProduct)
            toggleProductSelection('{{ $selectedProduct->id }}', '{{ $selectedProduct->name }}');
        @endif
        ...
        function initEventListeners() {
            ...
            document.querySelector('.grid .sm\\:grid-cols-2')?.addEventListener('click', handleProductClick); // For featured products if they exist
