    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-10 fade-in" style="animation-delay: 0.2s;">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {{-- Product Card --}}
                <div class="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col stagger-item border border-slate-100" style="animation-delay: {{ $index * 0.05 }}s;">
                    <a href="{{ route('products.show', $product) }}" class="block group">
                        <div class="relative h-48 bg-slate-200 overflow-hidden">
                            ...
                            {{-- Category Badge --}}
                            @if($product->category)
                            <span class="absolute top-2 right-2 bg-amber-100 text-amber-800 text-xs font-semibold px-2 py-0.5 rounded-full">
                    ...
                    <div class="p-4 flex flex-col flex-grow">
                        <h3 class="text-base font-semibold text-slate-800 mb-2 line-clamp-2 h-12 group-hover:text-[#99765c] transition-colors">
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-3">
                            ...
                            <a href="{{ route('quote-request.create', ['product_id' => $product->id]) }}"
                               class="w-full sm:w-auto text-center brand-bg hover:brand-bg-dark text-white text-xs font-bold py-2 px-4 rounded-md transition duration-300 shadow-sm hover:shadow-md">
                                Minta Penawaran
                            </a>
                        </div>
                    </div>
            ...
            <div class="mt-10 fade-in" style="animation-delay: 0.5s;">
                {{-- Assuming standard Tailwind pagination --}}
                {{ $products->links() }}
            </div>
            @endif
        </div>
        ...
        {{-- Contact Us Section --}}
        <div class="mt-16 bg-gradient-to-r from-slate-700 to-slate-900 text-white rounded-lg shadow-xl overflow-hidden fade-in" style="animation-delay: 0.6s;">
...
@push('scripts')
{{-- Assume products-modern.js handles filter toggle and back-to-top --}}
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        resetBtn?.addEventListener('click', () => {
            ...
            // Optionally submit the form to reload or just clear fields
            filterForm?.submit(); // Submit to show all products
        });
