...
Hingga saat ini, kami terus berinovasi dan berkembang untuk menjadi supplier bahan bangunan terdepan dengan komitmen pada kualitas dan kepuasan pelanggan.');
    ...
    public function contact()
    {
        ...
        // Ambil koordinat dari setting
        $latitude = Setting::getValue('lokasi_lat', '-7.4478');
        $longitude = Setting::getValue('lokasi_lng', '112.7183');

        // Alternatif: gunakan map_coordinates jika tersedia
        $mapCoordinates = Setting::getValue('map_coordinates', '');
        if (!empty($mapCoordinates)) {
            list($lat, $lng) = explode(',', $mapCoordinates);
            $latitude = $lat;
            $longitude = $lng;
        }
        ...
        $contactInfo = [
            ...
            'address_encoded' => urlencode($cleanAddress),
            'latitude' => $latitude,
            'longitude' => $longitude,
            'coordinates' => $latitude . ',' . $longitude,
            'phone' => Setting::getValue('telepon', '+62 812 3456 7890'),
            'mobile' => Setting::getValue('mobile', '0812-3456-7890'),
            'email' => Setting::getValue('email_kontak', 'info@modernsentra.com'),
            'hours' => Setting::getValue('jam_operasional', 'Senin - Sabtu: 08:00 - 17:00'),
        ];

        return view('contact-minimalist', compact('contactInfo'));
    }
}
