...
<div class="py-16 sm:py-20 bg-slate-50">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12 md:mb-16">
            ...
            <p class="text-base sm:text-lg text-slate-600 max-w-3xl mx-auto mb-6">
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-3xl mx-auto text-left">
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        <h3 class="text-sm font-medium text-blue-800">Informasi Permintaan Penawaran</h3>
                        <div class="mt-2 text-sm text-blue-700 space-y-1">
                            <p>Tim kami akan merespons permintaan Anda dalam waktu <span class="font-semibold">1x24
                                    jam</span> pada jam kerja (Senin-Jumat, 08:00-17:00 WIB).</p>
                        </div>
                    </div>
                </div>
            </div>
...
{{-- Partial for Product Item (Included via @include) --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Use a local object within this scope for managing selected products
        ...
        // --- Initialization ---
        console.log("Initializing Quote Form Script...");
        initEventListeners();
        preselectInitialProduct(); // Attempt pre-selection after listeners are set
        updateSelectedProductsUI(); // Initial UI setup based on potential pre-selection
        ...
        // --- Pre-selection Logic ---
        function preselectInitialProduct() {
            ...
            console.log(`Attempting pre-selection for ID: ${initialId}, Name: ${initialName}`);
            ...
            if (initialId && initialName) {
                ...
                if (initialProductElement) {
                    console.log("Found initial product element in DOM.");
                    toggleProductSelection(initialId, initialName, initialProductElement);
                } else {
                    // If element not found immediately (e.g., in collapsed category), add to state directly
                    console.log("Initial product element not found in DOM, adding directly to state.");
                    selectedProducts[initialId] = { id: initialId, name: initialName, quantity: 1 };
                    // updateSelectedProductsUI() will be called at the end of initialization
                }
            } else {
                console.log("No initial product ID/Name found in data attributes.");
            }
        }
        ...
        function toggleProductSelection(productId, productName, element = null) {
            ...
            // If called for pre-selection and element is null, we rely on the state being set already.
            // If called by click and element is null, something is wrong.
            if (!element && !selectedProducts[productId]) {
                console.warn(`Product item element not found for ID ${productId} and not already selected. Cannot toggle.`);
                return;
            }
            ...
            if (selectedProducts[productId]) {
                console.log(`Deselecting ${productId}`);
                ...
                console.log(`Selecting ${productId}`);
            ...
            console.log("Current selectedProducts state:", JSON.stringify(selectedProducts));
            updateSelectedProductsUI(); // Update UI after every toggle
        }

        function updateSelectedProductsUI() {
            console.log("Updating selected products UI...");
            const productIds = Object.keys(selectedProducts);
            const count = productIds.length;
            console.log(`Found ${count} selected products.`);
            ...
            if (count === 0) {
                ...
                console.log("Selected products UI updated (empty).");
                return;
            }
            ...
            productIds.forEach(id => {
                ...
                if (productItem) {
                    ...
                    console.log(`Could not find product item element for ID ${id} during UI update.`);
                }
            });
            console.log("Selected products UI updated.");
        }
        ...
        function handleQuantityInputChange(event) {
            ...
            if (selectedProducts[productId]) {
                ...
                updateSelectedProductsUI(); // Update UI to reflect quantity change
            }
        }
        ...
        function navigateToStep(step) {
            if (step === 1) {
                ...
                elements.progressConnector.classList.add('from-[#99765c]', 'via-[#99765c]', 'to-[#99765c]');
        ...
        // Assign update function to window *after* it's defined
        window.updateSelectedProductsUI = updateSelectedProductsUI;
        window.selectedProductsInitialized = true; // Flag initialization complete
